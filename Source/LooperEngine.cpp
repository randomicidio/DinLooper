                                ...
                                int recordingStopSample)
{
    ...
    if ((state == State::Playing || state == State::Overdubbing)
        && recordedSamples > 0)
    {
        const auto activeLayers = layerCount.load(std::memory_order_relaxed);
                    ...
                    if (slot >= 0)
                    {
                                ...
                                std::memory_order_acquire))
                        {
                            if (channel < layerBuffer->getNumChannels())
                                mixedSample += layerBuffer->getSample(channel, position);
                        }
                    }
                }
...
void LooperEngine::pressRec()
{
    ...
    case State::Playing:
    {
        ...
        if (layerCount.load(std::memory_order_relaxed) < maximumLayers)
        {
    ...
    case State::Overdubbing:
        if (recordingLayerIndex >= 0 && overdubSamplesWritten > 0)
        {
            ...
            const auto targetLayer =
                layerCount.load(std::memory_order_relaxed);
...
void LooperEngine::pressRedo()
{
    if (undoCount > 0)
    {
        ...
        if (currentState.load(std::memory_order_relaxed) == State::Idle)
            currentState = State::Playing;
    }
}
